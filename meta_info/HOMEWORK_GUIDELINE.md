...
*   **Kiểm chứng thực tế (30% số điểm):** Cung cấp đầy đủ minh chứng chạy thử nghiệm thành công (Link chia sẻ cuộc hội thoại hoặc bản sao cuộc hội thoại dạng văn bản/markdown block), kết quả hiển thị mượt mà, thân thiện và không bị crash logic khi gặp dữ liệu thử nghiệm dị biệt.
...
*   Sinh viên nộp bài dưới dạng text, khuyến khích sử dụng **một file Markdown duy nhất (.md)**.
...
3.  **Nội dung Bài 5 (Sáng tạo):**
    ...
    *   **Bắt buộc:** Cung cấp **Link chia sẻ cuộc hội thoại (Share Link)** từ ChatGPT/Claude hoặc sao chép đầy đủ nội dung cuộc hội thoại dưới dạng văn bản (text log/markdown block) trong file bài làm để minh họa kết quả chạy thực tế.

> [!WARNING]
> Mọi bài nộp không cung cấp Link chia sẻ cuộc hội thoại hoặc bản sao cuộc hội thoại bằng văn bản đối với các phần chạy thử nghiệm sẽ bị AI chấm bài đánh giá là không hoàn thành phần kiểm chứng thực tế và bị trừ 100% số điểm của phần đó để đảm bảo tính trung thực học thuật. Tuyệt đối không chấp nhận ảnh chụp màn hình (screenshot) làm minh chứng chạy thực tế.
