...
* Yêu cầu AI đề xuất cấu trúc dữ liệu tối ưu hơn (ví dụ: chuyển từ mảng sang Map/Hash Table) để giảm thời gian tìm kiếm từ $O(N)$ xuống $O(1)$.
...
### Mục tiêu demo
Minh họa quy trình sử dụng AI để phân tích và tối ưu hóa một thuật toán duyệt mảng lồng nhau có hiệu năng kém sang một cấu trúc phẳng tối ưu hơn trong JavaScript.

### Điều kiện chuẩn bị
* Một đoạn mã nguồn JavaScript chứa hai vòng lặp lồng nhau để tìm kiếm các sản phẩm trong kho của các cửa hàng.
* Công cụ AI hỗ trợ lập trình.

### Các bước thực hiện
1. **Bước 1:** Chuẩn bị mã nguồn chưa tối ưu có độ phức tạp cao.
2. **Bước 2:** Soạn Prompt yêu cầu AI tối ưu hiệu năng và giải thích nguyên lý.
3. **Bước 3:** Nhận mã nguồn đã tối ưu từ AI và tiến hành phân tích sự thay đổi.
4. **Bước 4:** Kiểm tra tính đúng đắn của logic mới để đảm bảo kết quả đầu ra không thay đổi.

### Prompt sử dụng
```text
Hãy tối ưu hóa hàm tìm kiếm sản phẩm theo danh mục dưới đây để cải thiện hiệu năng xử lý khi danh sách cửa hàng và danh mục sản phẩm tăng lên rất lớn. Giải thích rõ độ phức tạp thuật toán (Time Complexity) trước và sau khi tối ưu.
```

### Mã nguồn chưa tối ưu
```javascript
function searchProductsByCategory(stores, targetCategory) {
    let results = [];
    for (let i = 0; i < stores.length; i++) {
        let products = stores[i].inventory;
        for (let j = 0; j < products.length; j++) {
            if (products[j].category === targetCategory) {
...
### Mã nguồn tối ưu hóa do AI đề xuất
```javascript
function searchProductsByCategoryOptimized(stores, targetCategory) {
    return stores.flatMap(store => store.inventory).filter(product => product.category === targetCategory);
}
```

### Giải thích mã nguồn
* **Trước khi tối ưu:** Thuật toán sử dụng 2 vòng lặp lồng nhau để duyệt qua từng cửa hàng và từng sản phẩm trong kho của cửa hàng đó. Độ phức tạp thời gian là $O(S \times P)$ (với $S$ là số lượng cửa hàng và $P$ là số lượng sản phẩm trung bình mỗi cửa hàng). Khi quy mô hệ thống tăng lên hàng nghìn cửa hàng, CPU sẽ bị quá tải do số lượng phép toán tăng theo cấp số nhân.
* **Sau khi tối ưu:** Hàm sử dụng phương thức tích hợp sẵn `flatMap` để phẳng hóa toàn bộ danh sách sản phẩm từ tất cả cửa hàng thành một mảng duy nhất chỉ với một lượt duyệt, sau đó áp dụng `filter` để lọc ra sản phẩm thuộc danh mục yêu cầu. Độ phức tạp thời gian giảm xuống mức tuyến tính $O(N)$ (với $N$ là tổng số sản phẩm trong hệ thống), giúp CPU xử lý nhanh hơn đáng kể.
